...
function lunarYear(year) {
    const sYear = year.toString().split("");
    return gan[sYear[3]] + zhi[year % 12];
}
 ...
 * @returns {string}
 */
function ganText(year) {
    return gan[ganId(year)];
}
 ...
 * @returns {string}
 */
function zhiText(year) {
    return zhi[zhiId(year)];
}
...
function lunarMonthDays(year, month) {
    const monthData = lunarMonths(year);
...
function lunarYearDays(year) {
    const monthArray = lunarYearMonths(year);
...
function leapMonth(year) {
    const yearData = lunarInfo[year - MIN_YEAR];
    return yearData[0];
}
...
function lunarByBetween(year, between) {
    var lunarArray = [], yearMonth = [], t = 0, e = 0, _leapMonth = 0, m = '';
    if (between == 0) {
        t = 1;
        e = 1;
        m = '正月';
        ...
        m = (_leapMonth != 0 && t == _leapMonth + 1)
            ? ('闰' + chineseMonth(t - 1))
            : chineseMonth(((_leapMonth != 0 && _leapMonth + 1 < t) ? (t - 1) : t));
    }
    ...
    lunarArray.push(_leapMonth); //闰几月
    return lunarArray;
}
